...
export default function ChrisFlowersRegistrationPage() {
  const certificateRef = useRef<HTMLDivElement>(null);
  const [isSending, setIsSending] = useState(false);
  const [email, setEmail] = useState("");

  const [tenantName, setTenantName] = useState("Chris Flowers");
  const [propertyAddress, setPropertyAddress] = useState("260 W 18th St, San Bernadino, CA 92405");
  const [dateIssued, setDateIssued] = useState("June 18, 2026");
  const [registrationId, setRegistrationId] = useState("REG-2026-0851");
  ...
  return (
    ...
    >
      <div
        className="no-print"
        style={{ maxWidth: 860, margin: "0 auto 24px", display: "flex", gap: 12, flexDirection: "column" }}
      >
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button type="button" onClick={handleSendEmail} disabled={isSending}
            style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 8, background: "#0f1f3d", color: "#fff", border: "none", cursor: isSending ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 600, opacity: isSending ? 0.7 : 1 }}>
            {isSending ? <><RefreshCw size={15} className="spin-icon" /> Sending…</> : <><Send size={15} /> Send to Tenant</>}
          </button>
          <button type="button" onClick={handleDownload}
            style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 8, background: "#b8943a", color: "#fff", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
            <Download size={15} /> Download PDF
          </button>
          <button type="button" onClick={() => window.print()}
            style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 8, background: "#fff", color: "#0f1f3d", border: "1px solid #d4c9b0", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
      ...
      >
        ...
        <div style={{ position: "relative", zIndex: 1, padding: "36px 48px 0 48px" }}>
          <div style={{ textAlign: "center", borderBottom: "2px solid #0f1f3d", paddingBottom: 16, marginBottom: 20 }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 700, color: "#0f1f3d", margin: "0 0 4px 0", letterSpacing: "0.04em" }}>
              San Bernardino County Council — Housing & Property Division
            </h2>
            <p style={{ fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "#8a8070", margin: 0 }}>
              Office of Residential Registration · San Bernardino District
            </p>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, paddingBottom: 16, borderBottom: "1px solid #e8e4dc" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <div>
                <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 15, fontWeight: 700, color: "#0f1f3d" }}>Core Key Realty</div>
                ...
                <div style={{ fontSize: 10, color: "#8a8070", marginTop: 2 }}>Lic. No. CA-PMO-2024-3821 · San Bernardino County</div>
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ fontSize: 10, color: "#8a8070", marginTop: 4 }}>Date Issued: <strong style={{ color: "#1a1a1a" }}>{dateIssued}</strong></div>
            </div>
          </div>
          ...
          <div style={{ background: "#f7f4ef", border: "1px solid #d4c9b0", padding: "24px 28px", marginBottom: 28, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            ...
            <div>
              ...
              <p style={{ ...val, margin: 0 }}>San Bernardino County, California</p>
            </div>
            <div>
          ...
          <div style={{ fontSize: 14, color: "#374151", lineHeight: 1.9, marginBottom: 28, textAlign: "justify" }}>
            <p style={{ margin: "0 0 16px 0" }}>
              This is to certify, in accordance with the statutes and regulations of the <strong>State of California Housing Authority</strong> and the <strong>San Bernardino County Council — Housing & Property Division</strong>, that the residential property located at <strong>{propertyAddress}</strong> has been duly registered, reviewed, and officially allocated to the named tenant, <strong>{tenantName}</strong>.
            </p>
            <p style={{ margin: "0 0 16px 0" }}>
              This registration was processed and approved under the authority of Core Key Realty (Lic. No. CA-PMO-2024-3821), a fully licensed property management firm operating within the jurisdiction of San Bernardino County. All requisite documentation, background screening, lease compliance reviews, and statutory filings have been completed in accordance with the <em>California Civil Code</em> and the applicable local housing ordinances.
            </p>
            <p style={{ margin: 0 }}>
              This certificate grants <strong>{tenantName}</strong> the exclusive and legally recognized right of occupancy of the above-mentioned premises, subject to the terms and conditions of the duly executed lease agreement on file with this office. Any transfer, subletting, or modification of tenancy must be formally registered with the San Bernardino County Council — Housing & Property Division and Core Key Realty.
            </p>
          </div>
          ...
          <div style={{ background: "#0f1f3d", color: "#b8c4d4", fontSize: 11, padding: "12px 20px", letterSpacing: "0.06em", lineHeight: 1.6, marginBottom: 36, display: "flex", gap: 12, alignItems: "flex-start" }}>
            ...
            <span>
              <strong style={{ color: "#fff" }}>Legal Notice:</strong> This document has been issued under the authority of the San Bernardino County Council and the California Department of Housing and Community Development. Falsification or misuse of this certificate is a violation of California law and may result in civil or criminal penalties. This certificate is valid only for the named tenant and address above.
            </span>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 0, paddingBottom: 48 }}>
            <div style={{ textAlign: "center", minWidth: 180 }}>
              <div style={{ position: "relative", height: 70, marginBottom: 8 }}>
                <img src="/daniel-hall-sig.png" alt="Daniel Hall Signature" style={{ position: "absolute", bottom: 0, left: "50%", transform: "translateX(-50%)", height: 60, opacity: 0.85 }} />
              </div>
              <div style={{ borderTop: "1px solid #0f1f3d", paddingTop: 8 }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: "#0f1f3d", margin: "0 0 2px 0" }}>Daniel Hall</p>
                <p style={{ fontSize: 10, color: "#8a8070", textTransform: "uppercase", letterSpacing: "0.1em", margin: 0 }}>Attorney of Record</p>
                <p style={{ fontSize: 10, color: "#8a8070", margin: "2px 0 0 0" }}>Core Key Realty</p>
              </div>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
              ...
              >
                ...
                <p style={{ fontSize: 9, color: "#0f1f3d", fontWeight: 700, letterSpacing: "0.22em", textTransform: "uppercase", margin: "0 0 3px 0" }}>San Bernardino</p>
            ...
            <div style={{ textAlign: "center", minWidth: 180 }}>
              ...
              <div style={{ borderTop: "1px solid #0f1f3d", paddingTop: 8 }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: "#0f1f3d", margin: "0 0 2px 0" }}>Alejandra Sanchez</p>
                <p style={{ fontSize: 10, color: "#8a8070", textTransform: "uppercase", letterSpacing: "0.1em", margin: 0 }}>Lessor / Owner</p>
                <p style={{ fontSize: 10, color: "#8a8070", margin: "2px 0 0 0" }}>Lic. No. CA-2014-00192</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: "#0f1f3d", padding: "16px 48px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p style={{ color: "#b8c4d4", fontSize: 10, margin: 0, letterSpacing: "0.1em" }}>
            Core Key Realty · In partnership with Invitation Homes
          </p>
          <p style={{ color: "#6b7a94", fontSize: 10, margin: 0 }}>
            San Bernardino County Council · Housing & Property Division · San Bernardino, CA
          </p>
        </div>
